...
var require_add = __commonJS({
  "node_modules/before-after-hook/lib/add.js"(exports2, module2) {
    ...
    function addHook(state, kind2, name, hook) {
      var orig = hook;
      if (!state.registry[name]) {
        state.registry[name] = [];
      }
  ...
  "node_modules/before-after-hook/index.js"(exports2, module2) {
    ...
    function bindApi(hook, state, name) {
      ...
      ["before", "error", "after", "wrap"].forEach(function(kind2) {
        var args = name ? [state, kind2, name] : [state, kind2];
...
var require_dist_node12 = __commonJS({
  "node_modules/@octokit/rest/dist-node/index.js"(exports2, module2) {
    ...
    module2.exports = __toCommonJS(index_exports);
    var import_core28 = require_dist_node8();
    ...
    var VERSION2 = "20.1.2";
    var Octokit2 = import_core28.Octokit.plugin(
...
try {
  ...
  const invalid = parsed.filter((s2) => !validSeverities.includes(s2));
  if (invalid.length > 0) {
  ...
  failOn = /* @__PURE__ */ new Set(["high", "critical", "security"]);
}
...
function isSensitiveFile(filename) {
  return SENSITIVE_FILE_PATTERNS.some((p2) => p2.test(filename));
}
...
var ALLOWED_FILENAMES = [/^Dockerfile$/i, /^Dockerfile\./i, /^Makefile$/i, /^package\.json$/i];
function isAllowedFile(filename) {
  const basename2 = filename.split("/").pop();
  return ALLOWED_FILE_EXTENSIONS.some((r2) => r2.test(filename)) || ALLOWED_FILENAMES.some((r2) => r2.test(basename2));
}
...
function asMarkdown(review) {
  ...
  if (!review.issues.length) {
    ...
    for (const [i2, iss] of review.issues.entries()) {
      ...
      lines.push(
        `- **${i2 + 1}. [${iss.severity.toUpperCase()}] ${escapedTitle}** \u2014 \`${iss.file}${iss.line ? `:${iss.line}` : ""}\`
...
function truncateComment(text, maxLen = 6e4) {
...
function extractIssuesFromText(text) {
  ...
  if (!riskMatch && issues.length > 0) {
    const hasCritical = issues.some((i2) => i2.severity === "critical" || i2.severity === "security");
    const hasHigh = issues.some((i2) => i2.severity === "high");
    const hasMedium = issues.some((i2) => i2.severity === "medium");
    if (hasCritical) {
...
${marker}`;
    ...
    if (shouldFail) {
      console.error(
        ...
        parsed.issues.filter((i2) => failOn.has(i2.severity)).map((i2) => i2.title)
      );
      process.exit(1);
    } else {
